...
async def get_tunnel_providers():
    """List supported tunnel provider profiles in rollout order."""
    providers = list_tunnel_projects()
    return {
        "providers": providers,
        "count": len(providers),
    }
...
async def create_tunnel(tunnel: TunnelCreate, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    normalized_core, normalized_type, normalized_spec = normalize_tunnel_payload(
        tunnel.core,
        tunnel.type,
        tunnel.spec or {},
        attach_default_project=True,
    )
    if (
        normalized_core != tunnel.core
        or normalized_type != tunnel.type
        or normalized_spec != (tunnel.spec or {})
    ):
        logger.info(
            "Normalized tunnel payload: core %s -> %s, type %s -> %s, project_id=%s",
            tunnel.core,
            normalized_core,
            tunnel.type,
            normalized_type,
            normalized_spec.get("project_id"),
        )
...
async def update_tunnel(
    ...
    if tunnel_update.spec is not None:
        normalized_core, normalized_type, normalized_spec = normalize_tunnel_payload(
            tunnel.core,
            tunnel.type,
            tunnel_update.spec or {},
            attach_default_project=False,
        )
        spec_changed = (
            normalized_spec != tunnel.spec
            or normalized_core != tunnel.core
            or normalized_type != tunnel.type
        )
        if normalized_core != tunnel.core or normalized_type != tunnel.type:
            logger.info(
                "Tunnel %s update normalized: core %s -> %s, type %s -> %s",
                tunnel_id,
                tunnel.core,
                normalized_core,
                tunnel.type,
...
async def apply_tunnel(tunnel_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if (
        ...
        logger.info(
            "Apply normalization for tunnel %s: core %s -> %s, type %s -> %s",
            tunnel.id,
            tunnel.core,
            normalized_core,
            tunnel.type,

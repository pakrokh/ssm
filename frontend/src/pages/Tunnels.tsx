...
interface TunnelProjectProfile {
  id: string
  label: string
  core: 'gost' | 'backhaul' | 'rathole' | 'frp'
  repo: string
  defaultType: string
...
const DEFAULT_TUNNEL_PROJECTS: Record<string, TunnelProjectProfile> = {
  trusttunnel: {
    id: 'trusttunnel',
    label: 'TrustTunnel',
    core: 'gost',
  ...
  backhaul_script: {
    ...
    defaultType: 'tcpmux',
    supportedTypes: ['tcp', 'udp', 'ws', 'wsmux', 'tcpmux'],
    defaultPorts: '443',
  },
  icmp_tun: {
    id: 'icmp_tun',
...
const getProjectProfile = (
  projectId: string,
  projectCatalog: Record<string, TunnelProjectProfile> = DEFAULT_TUNNEL_PROJECTS
): TunnelProjectProfile => {
  const fallback = projectCatalog.trusttunnel || DEFAULT_TUNNEL_PROJECTS.trusttunnel
  return projectCatalog[projectId] || fallback
}

const normalizeProvider = (raw: any): TunnelProjectProfile | null => {
  if (!raw || typeof raw !== 'object') return null
  if (!raw.id || !raw.label || !raw.core) return null
  if (!Array.isArray(raw.supported_types)) return null
  ...
  return {
    ...
    core: raw.core,
    repo: raw.repo || '',
    defaultType: raw.default_type || 'tcp',
    supportedTypes: raw.supported_types,
    defaultPorts: raw.default_ports || '443',
    ...
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="space-y-3">
        {tunnels.map((tunnel) => {
          ...
          const projectLabel = tunnel.spec?.project_label || tunnel.spec?.project_id || tunnel.core
...
const AddTunnelModal = ({ nodes, servers, onClose, onSuccess }: AddTunnelModalProps) => {
  ...
  const initialProfile = getProjectProfile(initialProject, DEFAULT_TUNNEL_PROJECTS)
  const [formData, setFormData] = useState({
    name: '',
    project: initialProject,
    core: initialProfile.core,
    type: initialProfile.defaultType,
    node_id: '',
    foreign_node_id: '',
    iran_node_id: '',
    ports: initialProfile.defaultPorts,  // Comma-separated ports (e.g., "8080,8081,8082")
    remote_ip: '127.0.0.1',
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        const response = await api.get('/tunnels/providers')
        const providers = Array.isArray(response?.data?.providers) ? response.data.providers : []
        const parsedProfiles = providers
          .map((provider: any) => normalizeProvider(provider))
          .filter(Boolean) as TunnelProjectProfile[]

        if (parsedProfiles.length === 0) {
          return
        }

        const nextCatalog: Record<string, TunnelProjectProfile> = {}
        ...
        setFormData((prev) => {
          const activeId = nextCatalog[prev.project] ? prev.project : nextOrder[0]
          const activeProfile = getProjectProfile(activeId, nextCatalog)
          const nextType = activeProfile.supportedTypes.includes(prev.type)
            ? prev.type
          ...
          return {
            ...
            type: nextType,
            ports: prev.project === activeId ? prev.ports : activeProfile.defaultPorts,
          }
        })
      } catch (error) {
        console.warn('Failed to load provider catalog, using defaults:', error)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const projectProfile = getProjectProfile(formData.project, projectCatalog)
      const tunnelCore = projectProfile.core
      let spec = getSpecForType(tunnelCore, formData.type)
      let tunnelType = formData.type
      
      spec.use_ipv6 = formData.use_ipv6 || false
      spec.project_id = formData.project
      spec.project_label = projectProfile.label
      spec.project_repo = projectProfile.repo
      if (projectProfile.runtimeMode) {
        spec.runtime_mode = projectProfile.runtimeMode
      }
      ...
      if (tunnelCore === 'gost' && (formData.type === 'tcp' || formData.type === 'udp' || formData.type === 'ws' || formData.type === 'wsmux' || formData.type === 'grpc' || formData.type === 'tcpmux')) {
  ...
  const handleProjectChange = (project: string) => {
    const profile = getProjectProfile(project, projectCatalog)
    const fallbackType = profile.core === 'backhaul' ? backhaulState.transport : profile.defaultType
    const nextType = profile.supportedTypes.includes(formData.type) ? formData.type : fallbackType
    setFormData({
      ...formData,
      project,
      core: profile.core,
      type: nextType,
      ports: profile.defaultPorts,
    })
  }
  ...
  const activeProjectProfile = getProjectProfile(formData.project, projectCatalog)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] overflow-auto">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 w-full max-w-xl my-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                {projectOrder.map((projectId) => {
                  const profile = projectCatalog[projectId]
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white"
              >
                {activeProjectProfile.supportedTypes.map((type) => (
          ...
          {formData.core === 'gost' && (formData.type === 'tcp' || formData.type === 'udp' || formData.type === 'ws' || formData.type === 'wsmux' || formData.type === 'grpc' || formData.type === 'tcpmux') && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t.tunnels.remoteIP}
